...
from .util import sort_dataset,save_params
...
def knn(train_features, train_labels, test_features, test_labels, k=10):
...
def nearsub(train_features, train_labels, test_features, test_labels, 
            num_classes, n_comp=20, return_pred=False):
    train_scores, test_scores = [], []
    classes = np.arange(num_classes)
    features_sort, _ = sort_dataset(
        train_features, train_labels, classes=classes, stack=False)
...
def nearsub_pca(train_features, train_labels, test_features, test_labels, num_classes, n_comp=20):
    scores_pca = []
    classes = np.arange(num_classes)
    ...
    return acc_pca,test_predict_pca
...
def baseline(train_features, train_labels, test_features, test_labels):
    test_models = {'log_l2': SGDClassifier(loss='log_loss', max_iter=10000, random_state=42),
...
def evaluate(eval_dir, method, train_features, train_labels, test_features, test_labels, **kwargs):
    if method == 'svm':
        ...
        acc_train, acc_test = nearsub_pca(train_features, train_labels, test_features, test_labels, **kwargs)
    acc_dict = {'train': acc_train, 'test': acc_test}
    save_params(eval_dir, acc_dict, name=f'acc_{method}')
    return acc_dict
